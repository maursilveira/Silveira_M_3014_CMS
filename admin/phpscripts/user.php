<?php
  function createUser($fname, $username, $password, $email, $level) {
    ...
    $userString = "INSERT INTO user (user_fname, user_name, user_pass, user_email, user_date, user_ip, user_level) VALUES ('${fname}', '${username}', '${password}', '${email}', NOW(), 'no', ${level})";
  ...
  function editUser($id, $fname, $username, $email, $password, $lastlogin) {
    ...
    if(!$lastlogin) {
      ...
      if($password) {
        ...
        $updateString = "UPDATE user SET user_fname = '{$fname}', user_name = '{$username}', user_email = '{$email}', user_pass = '{$cryptPassword}' WHERE user_id = {$id}";
      }
    ...
    // if user has a registered previous login
      ...
      //if password was not filled
        ...
        $updateString = "UPDATE user SET user_fname = '{$fname}', user_name = '{$username}', user_email = '{$email}' WHERE user_id = {$id}";
      }
    }
  ...
  function deleteUser($id) {
    ...
    $deleteString = "DELETE FROM user WHERE user_id = {$id}";
  ...
  function updateLogin($id, $ip) {
    include('connect.php');
    // store IP
    $updateIp = "UPDATE user SET user_ip = '{$ip}' WHERE user_id = {$id}";
    $updatequery = mysqli_query($link, $updateIp);

    // reset failed login sttempts
    $resetFail = "UPDATE user SET user_attempt_fail = 0 WHERE user_id = {$id}";
    $runReset = mysqli_query($link, $resetFail);

    // store date and time of last successful login
    $setTime = "UPDATE user SET user_last_login = NOW() WHERE user_id = {$id}";
